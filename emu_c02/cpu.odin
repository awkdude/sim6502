...
address_from_mode :: proc(cpu: ^Cpu, mode: Address_Mode) -> uint {
    ...
    case .dpx: ea = u16(cpu_advance_byte(cpu) + cpu.x)
    case .dpy: ea = u16(cpu_advance_byte(cpu) + cpu.y)
...
cpu_debug :: proc(cpu: ^Cpu) {
    ...
    buf: [128]u8
    fmt.bprintf(buf[:], "A: %2.0x, X: %2.0x, Y: %2.0x, SP: 01%2.0x, PC: %4.0x," +
    " P: %c%c%c%c%c%c%c (%v cc)\n", 
        cpu.a,
        cpu.x,
        cpu.y,
        cpu.sp,
