...
wait_frame_interval :: proc() {
    target_frame_interval_usec :: 16666
    @static previous_frame_time: time.Tick 
    now := time.tick_now()
    time_elapsed_duration := time.tick_diff(previous_frame_time, now)
...
main :: proc() {
    ...
    draw_context = draw.new_draw_context_sw({
        get_render_target_dpi = proc(draw_context: ^draw.Draw_Context) -> i32 {
            return 96
        },
    })
    app_context.draw_context = &draw_context
